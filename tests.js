...
describe("Quote App", () => {
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile("quotes.txt", "").catch(() => {});
  });
  after(async () => {
  ...
  it("should not log ALL quotes (don't console.log the array of lines)", async () => {
    const quoteArr = quotes.map(({ quote }) => quote);
    await Promise.all(
      quotes.map(({ quote, author }) => addQuote(quote, author))
    );
    const output = await getQuote();
    let included = 0;
    for (const quote of quoteArr) {
      if (output.includes(quote)) included++;
    }
    expect(included).to.eq(1);
  });
